...
Object.keys(optMap).map(ok => {
    ...
    str += Object.keys(ov.params).map(pk => {
        const pv = ov.params[pk];
        return `* **${pv.name}** - ${pk} => **Type**: ${pv.type}, **Default**: ${pv.default}`;
    }).join('\n');
    str += `\n\n[Sharp ${capitalizeFirstLetter(ok.replace('o_', ''))} Documentation](https://sharp.pixelplumbing.com/api-operation#${ok.toLowerCase().replace('o_', '')})`
    str += '\n\n';
});
